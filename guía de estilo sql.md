...
### 4.1 IDs — UUID obligatorio

**Las llaves primarias (`id`) SIEMPRE deben ser `uuid`.** No se permite `integer`, `bigint`, `serial` ni `GENERATED BY DEFAULT AS IDENTITY` como tipo de PK en tablas nuevas.

*   **PG18+**: `DEFAULT uuidv7()` — orden temporal, menor fragmentación de índice B-tree.
*   **PG ≤ 17**: `DEFAULT gen_random_uuid()` — built-in desde PG13, sin extensiones adicionales.
*   No usar `uuid_generate_v4()` (requiere extensión `uuid-ossp`, evitable).

```sql
-- PG18+
id uuid NOT NULL DEFAULT uuidv7() PRIMARY KEY

-- PG <= 17 (alternativa compatible)
id uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY
```

> **Regla de llaves foráneas:** Las columnas que referencian `id uuid` de otra tabla también deben ser de tipo `uuid`.
...
CREATE TABLE IF NOT EXISTS auditoria.log_auditoria (
    ...
    pk_value        text,                    -- valor del PK (como text)
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='app' AND table_name='personas') THEN
    EXECUTE $SQL$
      CREATE TABLE app.personas (
        -- id SIEMPRE uuid; usar uuidv7() en PG18+, gen_random_uuid() en PG<=17
        id                   uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  ...
  IF NOT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema='app' AND table_name='ordenes') THEN
    EXECUTE $SQL$
      CREATE TABLE app.ordenes (
        id                 uuid NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
        ref                uuid NOT NULL UNIQUE DEFAULT gen_random_uuid(),
        persona_id         uuid NOT NULL,     -- FK a personas.id (uuid)
...
SELECT util.ensure_fk(
  'app','ordenes','fk_ordenes__persona_id__personas',
  ARRAY['persona_id'],
  'app','personas',ARRAY['id'],
  TRUE
);
SELECT util.ensure_index_for_columns('app','ordenes','fki_fk_ordenes__persona_id', ARRAY['persona_id']);

COMMENT ON TABLE app.ordenes IS 'Órdenes asociadas a personas (FK por id uuid).';
COMMENT ON CONSTRAINT fk_ordenes__persona_id__personas ON app.ordenes IS 'Integra órdenes con personas (por PK uuid).';
...
*   [ ] **IDs: `id` SIEMPRE `uuid NOT NULL DEFAULT gen_random_uuid()` (o `uuidv7()` en PG18+). No se aceptan `integer`, `bigint`, `serial` ni `IDENTITY` como PK.**
*   [ ] **Las FKs que referencian `id uuid` son también de tipo `uuid`.**
