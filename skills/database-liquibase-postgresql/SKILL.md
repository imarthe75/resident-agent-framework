...
description: "Use when creating or updating PostgreSQL migration SQL for Liquibase with idempotent DDL/DML, audit triggers, naming conventions, and review checklist. Triggers: liquibase, changelog sql, migration script, postgres migration, idempotent sql, audit_biu, audit_aiud, uuid primary key, uuid pk, llave primaria uuid."
---
...
- **Las llaves primarias SIEMPRE deben ser `uuid`.** No se permite `integer`, `bigint`, `serial` ni `GENERATED BY DEFAULT AS IDENTITY` como tipo de `id`.
- **UUID Generation (obligatorio):**
  - PostgreSQL 18+: `DEFAULT uuidv7()` — preferido por orden temporal y menor fragmentación de índices B-tree.
  - PostgreSQL ≤ 17: `DEFAULT gen_random_uuid()` — built-in desde PG 13, no requiere extensiones.
  - No usar `uuid_generate_v4()` (requiere la extensión `uuid-ossp`, evitable).
- Las columnas de llave foránea que referencian a `id uuid` de otra tabla deben ser también de tipo `uuid`.
...
ALTER TABLE public.nombre_tabla
    -- id SIEMPRE uuid; usar gen_random_uuid() en PG<=17, uuidv7() en PG18+
...
-- 3. Llaves foraneas
-- ===========================
-- Nota: columna_fk debe ser uuid si tabla_padre.columna_padre es uuid (PK).
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1
        ...
        ALTER TABLE public.nombre_tabla
            ...
            FOREIGN KEY (columna_fk)         -- tipo: uuid
            REFERENCES public.tabla_padre(id); -- id siempre uuid
    END IF;
END $$;
...
-- ===========================
-- Nota: ON CONFLICT debe usar la clave de negocio (codigo), no el id uuid surrogate.
DO $$
BEGIN
    IF EXISTS (
        SELECT 1
        ...
        INSERT INTO public.nombre_tabla(codigo)
        VALUES ('CODIGO_BASE')
...
-- 3. id siempre uuid con DEFAULT gen_random_uuid() (o uuidv7() en PG18+).
-- 4. Las llaves foraneas que apunten a id son tambien de tipo uuid.
-- 5. audit_biu siempre activo.
-- 6. audit_aiud creado y deshabilitado por defecto.
...
- [ ] **La columna `id` es de tipo `uuid NOT NULL DEFAULT gen_random_uuid()` (o `uuidv7()` en PG18+). No se aceptan `integer`, `bigint`, `serial` ni `IDENTITY` como PK.**
- [ ] **Las columnas de llave foranea que referencian a `id uuid` son tambien de tipo `uuid`.**
